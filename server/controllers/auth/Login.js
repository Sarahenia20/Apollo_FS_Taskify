          ...
          .then(() => {
            ...
            if (isSuccessful && suspiciousCheck.isSuspicious) {
              ...
              return Promise.resolve().then(async () => {
                try {
                  const result = await sendSuspiciousLoginAlert(user, loginActivity);
          ...
          if (!user) {
            ...
            bcrypt.compare(req.body.password, user.password)
              .then(isMatch => {
                if (!isMatch) {
                  // Record failed login
                  recordLoginActivity(user, req, false)
                    .then(() => {
                  ...
                  if (user.twoFactorEnabled === true && !TEST_MODE) {
                    ...
                    recordLoginActivity(user, req, true, true)
                      .then(() => {
                    ...
                    recordLoginActivity(user, req, true)
                      .then(securityCheck => {
                        ...
                        const testLoginActivity = {
                          ...
                          suspiciousReason: ['Test reason - Force email alert for testing']
                        };

                        // Convert async function to promise chain
                        Promise.resolve().then(async () => {
                          try {
                            const emailResult = await sendSuspiciousLoginAlert(user, testLoginActivity);
                            console.log("Test email sending result:", emailResult);
                            
                            if (emailResult.previewUrl) {
                              console.log("📧 View test email at:", emailResult.previewUrl);
                            }
