...
const Roles = () => {
  ...
  const fetchRoles = async () => {
    try {
      ...
      // Get roles from API
      let apiRoles = response.data.data || [];
      ...
      const existingDefaultRoles = processedRoles
        .filter(role => defaultRoleNames.includes(role.name.toLowerCase()))
      ...
      setRoles([...processedRoles, ...additionalDefaultRoles]);
    } catch (error) {
  ...
  const handleSaveRole = async (newRole) => {
    try {
      ...
      setRoles([...roles, { ...response.data.data, isDefault: false }]);
