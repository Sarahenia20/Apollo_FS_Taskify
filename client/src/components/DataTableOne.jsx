...
const DataTableOne = ({ data, popupOpen, setPopupOpen }) => {
  ...
  // IMPORTANT: This function bypasses the DataTable library's event handling
  // by attaching direct event listeners to the buttons after the table is rendered
  ...
  const handleDelete = (e) => {
    ...
    if (window.confirm("Do you want to delete this user?")) {
      ...
      // Direct API call to delete the user
  ...
  // Direct event handler for edit buttons
  const handleEdit = (e) => {
    e.preventDefault();
    e.stopPropagation();
  ...
  // Re-attach event handlers whenever data changes
  useEffect(() => {
    ...
    <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
      <div className="data-table-common data-table-one max-w-full overflow-x-auto">
        <table className="table w-full table-auto" id="dataTableOne">
          ...
          <tbody>
            {Array.isArray(data) && data.map((userData, index) => (
              <tr key={userData._id || index}>
                ...
                <td className="flex gap-x-3 gap-y-3">
                  {/* Delete Button - with direct data attributes */}
                  <button
                  ...
                  {/* Edit Button - with direct data attributes */}
                  <button
