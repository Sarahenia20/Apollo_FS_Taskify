...
const RoleModal = ({ isOpen, onClose, onSave }) => {
  ...
  const generateDescription = (name) => {
    ...
    setTimeout(() => {
      ...
      setIsLoading(false);
    }, 700); // Slightly longer delay to simulate more complex processing
  };
  ...
  const handleSubmit = () => {
    ...
    const newRole = {
      ...
      avatar: roleName.substring(0, 1).toUpperCase()
    };
...
import { generateDescriptionAction } from '../redux/actions/roles';
...
const RoleModal = ({ isOpen, onClose, onSave }) => {
  ...
  const { generatedDescription } = useSelector(state => state.roles);
  ...
  // Use the generated description from Redux
  useEffect(() => {
    if (generatedDescription) {
      setRoleDescription(generatedDescription);
    }
  ...
  useEffect(() => {
    if (roleName.trim().length > 2) {
      setIsLoading(true);
      dispatch(generateDescriptionAction(roleName))
        .finally(() => {
          setIsLoading(false);
        });
    } else {
