...
export const CreateTeamAction = (form) => async (dispatch) => {
  ...
  try {
    ...
    if (Array.isArray(form.members)) {
      ...
      // If members is an array of objects with value property (from react-select)
    ...
    return true; // Return success value for the component to handle
  } catch (error) {
    console.error("Error creating team:", error);
    dispatch(setErrors(error.response?.data || { message: "An error occurred" }));
    dispatch(setRefresh(false));
    return false; // Return failure for the component to handle
  }
};
