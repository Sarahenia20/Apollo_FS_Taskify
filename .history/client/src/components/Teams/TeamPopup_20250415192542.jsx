...
const TeamPopup = ({ popupOpen, setPopupOpen, editingId, popup, onSuccess }) => {
  ...
  const onSubmitHandler = (e) => {
    ...
    // Prepare clean members array with proper user IDs
    const members = [];
    if (Array.isArray(form.members)) {
      form.members.forEach(member => {
        // Ensure we have a valid user ID, either as direct ID or from an object
    ...
    const hasCurrentUser = members.some(id => id === user._id);
    if (!hasCurrentUser) {
    ...
    const teamData = {
      ...
      members: members, // Send the properly formatted members array
