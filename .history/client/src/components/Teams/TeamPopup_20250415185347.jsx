...
const TeamPopup = ({ popupOpen, setPopupOpen, editingId, popup, onSuccess }) => {
  ...
  const onSubmitHandler = (e) => {
    ...
    // Prepare clean members array
    const members = [];
    if (Array.isArray(form.members)) {
      form.members.forEach(member => {
        if (member?.user) { // Only include members with valid user IDs
          members.push({
            user: member.user,
    ...
    // Include current user as admin if not already present
    const hasCurrentUser = members.some(m => m.user === user._id);
    if (!hasCurrentUser) {
      members.unshift({
        user: user._id,
        role: "ADMIN"
    ...
    const teamData = {
      ...
      members: members.map(m => m.user), // Send just the IDs
      pictureprofile: form.pictureprofile
    };
  
    console.log('Validated submission data:', teamData);
