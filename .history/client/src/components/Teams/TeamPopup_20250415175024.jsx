...
const TeamPopup = ({ popupOpen, setPopupOpen, editingId, popup, onSuccess }) => {
  ...
  // Initialize form with team data when editing or reset when creating new
  useEffect(() => {
    if (popupOpen) {
      if (editingId && currentTeam?._id === editingId) {
        ...
        // For new team, reset form and automatically add current user as admin
        clearForm();
      }
    }
  ...
  // Get filtered users based on search (excluding current user)
  ...
  const onSubmitHandler = (e) => {
    ...
    // Make sure form.members is an array
    const members = Array.isArray(form.members) ? form.members : [];
    
    // Ensure current user is in the members list as ADMIN if creating new team
    let finalMembers = [...members];
    if (!editingId) {
      const creatorExists = finalMembers.some(m => m.user === user._id);
      if (!creatorExists) {
        finalMembers.push({
          user: user._id,
          role: "ADMIN"
    ...
    if (form.pictureprofile && form.pictureprofile instanceof File) {
      formData.append('pictureprofile', form.pictureprofile);
    }
    
    console.log('Final submission members:', finalMembers); // Debug log
  
    if (editingId) {
      dispatch(UpdateTeamAction(formData, editingId, () => {
        if (onSuccess) onSuccess();
        clearForm();
      }));
    } else {
      dispatch(CreateTeamAction(formData, () => {
        if (onSuccess) onSuccess();
        clearForm();
      }));
    }
        ...
        {!refresh ? (
          <form onSubmit={onSubmitHandler}>
            ...
            <div className="mb-4">
              ...
              />
              {errors.Name && (
                <div className="text-sm text-meta-1">{errors.Name}</div>
              )}
            </div>
            ...
            <div className="mb-5">
              ...
              />
              {errors.description && (
                <div className="text-sm text-meta-1">{errors.description}</div>
              )}
            </div>
            ...
            <div className="mb-5">
                      ...
                      >
                        <div className="h-6 w-6 rounded-full bg-primary text-white flex items-center justify-center overflow-hidden">
                          {memberDetails.picture ? (
                            ...
                            <span className="text-xs">{memberDetails.fullName?.charAt(0) || member.user.charAt(0)}</span>
                          )}
                        </div>
                        <span className="text-sm">{memberDetails.fullName || `User ${member.user.substring(0, 6)}...`}</span>
              ...
              {errors.members && (
                <div className="mt-2 text-sm text-meta-1">{errors.members}</div>
              )}
            </div>
                        ...
                        <div className="col-span-7 flex items-center gap-3">
                          <div className="h-10 w-10 rounded-full overflow-hidden bg-primary text-white flex items-center justify-center">
                            {memberDetails.picture ? (
                              ...
                              <span>{memberDetails.fullName?.charAt(0) || member.user.charAt(0)}</span>
                            )}
                          </div>
                          <div>
                            <h5 className="font-medium text-black dark:text-white">
                              {memberDetails.fullName || `User ${member.user.substring(0, 6)}...`}
                              {(member.user === user._id) && (
                                <span className="ml-2 text-xs text-primary">{(!editingId) ? "(Creator)" : "(You)"}</span>
                              )}
                            </h5>
                            {memberDetails.email && (
                              <p className="text-xs text-body-color">{memberDetails.email}</p>
                            )}
                          </div>
                        </div>
                        <div className="col-span-5 flex items-center justify-end">
                          <select
                            ...
                            disabled={!editingId && member.user === user._id}
