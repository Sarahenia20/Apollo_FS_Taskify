...
const TeamPopup = ({ popupOpen, setPopupOpen, editingId, popup, onSuccess }) => {
  ...
  const onSubmitHandler = (e) => {
    ...
    // Extract just the user IDs from members
    const memberIds = form.members.map(m => m.user);
  
    const teamData = {
      Name: form.Name.trim(),
      description: form.description,
      members: memberIds, // Send array of IDs only
    ...
    if (editingId) {
      dispatch(UpdateTeamAction(editingId, teamData, () => {
